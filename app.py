# app.py - Gym Bro v4.0 (Goal wizard, AI program, calendar, streaks, beautiful UI)
    ...
    if gym_bro.current_program:
        ...
        if today_program:
            ...
            if st.button("Log This Workout", type="primary"):
                # Pre-fill the workout tab (we'll switch tab programmatically? Not directly possible, so just navigate)
                st.session_state.current_exercises = []
                for ex in today_program["exercises"]:
...
# --- TAB 2: LOG WORKOUT (unchanged from previous good version) ---
with tab2:
...
st.markdown("---")
st.caption(f"Gym Bro v4.0 | User: {username} | We go jim! 🏋️")